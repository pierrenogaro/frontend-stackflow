...
const fetchAnswer = async () => {
  ...
  error.value = null;
  try {
    const response = await axios.get(`https://stackflow.pierrenogaro.com/answers/${answerId}/?t=${new Date().getTime()}`);
...
const updateAnswer = async () => {
  ...
  try {
    await axios.put(
        `https://stackflow.pierrenogaro.com/answer/update/${answerId}/`,
        answer.value,
